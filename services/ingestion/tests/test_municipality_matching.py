...
class MunicipalityMatchingTests(unittest.TestCase):
    ...
    def test_suggestions_match_exact_city_and_state_and_flag_ambiguous_rows(self, fetch_catalog, fetch_rows):
        ...
        self.assertEqual(result["matches"][3]["status"], "unmatched")
    ...
    @patch("services.ingestion.app.main.fetch_rows_for_municipality_resolution")
    @patch("services.ingestion.app.main.fetch_ibge_municipalities")
    def test_confirmed_matches_register_official_municipality_and_preserve_other_normalized_fields(self, fetch_catalog, fetch_rows, supabase_url, supabase_headers, post_request, patch_request):
        fetch_catalog.return_value = self.municipalities
        fetch_rows.return_value = [{"id": 15, "row_number": 1, "raw_row": {"municipio": "Campinas (SP)"}, "normalized_row": {"value": "12.5", "reference_year": "2024"}}]
        ...
        self.assertEqual(post_request.call_args.kwargs["json"], [{"ibge_code": "3509502", "name": "Campinas", "state": "SP"}])
        self.assertEqual(post_request.call_args.kwargs["headers"]["Content-Profile"], "municipal")
        self.assertEqual(patch_request.call_args.kwargs["json"]["normalized_row"], {"value": "12.5", "reference_year": "2024", "municipality_ibge_code": "3509502"})
    ...
    def test_confirmation_rejects_a_code_that_does_not_match_the_source_name(self, fetch_catalog, fetch_rows, post_request):
        ...
        with self.assertRaises(HTTPException) as caught_error:
            main.apply_municipality_matches("import-id", main.MunicipalityResolution(municipality_field="municipio", matches=[main.MunicipalityMatch(row_number=1, ibge_code="3550308")]))
