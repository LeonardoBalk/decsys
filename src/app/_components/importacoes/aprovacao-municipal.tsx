...
export function MunicipalApproval({ importId, sourceProfile }: MunicipalApprovalProps) {
  ...
  return <section className={styles.analysisPanel}>
    ...
    {sourceProfile.indicator_recommendations?.length ? <div className={styles.indicatorRecommendations}>{sourceProfile.indicator_recommendations.map((recommendation) => <div className={styles.indicatorRecommendation} key={recommendation.code}><div><strong>{recommendation.name}</strong><p>{recommendation.unit} · campo sugerido: {recommendation.value_field}</p></div></div>)}</div> : null}
    <a className={styles.sourceLink} href="/indicadores">Cadastrar ou organizar indicadores</a>
    ...
    </form> : <p className={styles.profileGuidance}>Cadastre o primeiro indicador na área de Indicadores para liberar a gravação municipal.</p>}
  </section>;
}
