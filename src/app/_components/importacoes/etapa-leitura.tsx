...
export function EtapaLeitura({ sourceProfile, onAdvance, onSelectSheet, isAnalyzing, feedbackMessage }: EtapaLeituraProps) {
  return <StepScreen eyebrow="ETAPA 2 DE 3" title="Leitura" description="Colunas, qualidade e sugestões de mapeamento.">
    ...
    {sourceProfile.sheets && sourceProfile.sheets.length > 1 ? <section className={styles.analysisPanel}><div className={styles.sectionHeading}><div><h2>Selecionar aba</h2><span>Escolha a parte da planilha que deseja revisar. Abas sem conteúdo não podem ser encaminhadas.</span></div></div><div className={styles.sheetSelector}>{sourceProfile.sheets.map((sheet) => { const isSelected = sourceProfile.selected_sheet === sheet.name; const isEmpty = sheet.has_data === false; return <button aria-pressed={isSelected} className={`${styles.sheetButton} ${isSelected ? styles.selectedSheetButton : ""}`} disabled={isAnalyzing || isEmpty} key={sheet.name} onClick={() => onSelectSheet(sheet.name)} type="button">{isEmpty ? `${sheet.name} · sem dados` : `${sheet.name} · ${sheet.rows.toLocaleString("pt-BR")} linhas`}</button>; })}</div></section> : null}
